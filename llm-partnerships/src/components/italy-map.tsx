...
import italyGeojsonRaw from "../../data/italy.json"
...
const italyGeojson = italyGeojsonRaw as any
const italyFeature = italyGeojson.features?.[0]
...
const mapCopy = {
  fr: {
    ...
    reset: "Reset",
    aria: "Carte d'Italie avec points des universités",
    selection: "Sélection :",
  ...
  en: {
    ...
    reset: "Reset",
    aria: "Map of Italy with university points",
    selection: "Selection:",
  ...
  es: {
    ...
    reset: "Reset",
    aria: "Mapa de Italia con puntos universitarios",
    selection: "Selección:",
  ...
  de: {
    ...
    reset: "Zurücksetzen",
    aria: "Italienkarte mit Universitätspunkten",
    selection: "Auswahl:",
  ...
  it: {
    ...
    reset: "Reset",
    aria: "Mappa dell'Italia con punti universitari",
    selection: "Selezione:",
...
export function ItalyMap({
  points,
  selectedItalianUniversity,
  onSelect,
  className,
  language = "fr",
}: {
  points: FrenchUniversityPoint[]
  selectedItalianUniversity?: string
  ...
  const { svgPath, projectPoint } = React.useMemo(() => {
    ...
    const d = italyFeature ? pathGen(italyFeature) : null
        ...
        <div className="relative">
          ...
          >
            ...
            <g transform={`translate(${transform.x} ${transform.y}) scale(${transform.k})`}>
              {svgPath ? (
                <>
                  <path d={svgPath} fill="url(#itFill)" filter="url(#itSoftShadow)" />
              ...
              {displayPoints.map(({ p, x, y }) => {
                const selected = p.frenchUniversity === selectedItalianUniversity
      ...
      {selectedItalianUniversity ? (
        <div className="mt-3 text-sm text-muted-foreground">
          ...
          <span className="font-medium text-foreground">
            {translateDataText(selectedItalianUniversity, language)}
          </span>
        </div>
      ) : (
