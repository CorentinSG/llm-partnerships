...
import {
  ...
  getItalianFilterOptions,
  getItalianUniversitiesPoints,
} from "@/lib/italy-data"
import {
  reliabilityCopy,
  translateDataText,
...
const italyCostSimulatorOrigin = {
  flights: {
    fr: "Billets d'avion Italie - États-Unis",
    en: "Italy - U.S. flights",
    es: "Vuelos Italia - Estados Unidos",
    de: "Flüge von Italien nach USA",
    it: "Voli Italia-USA",
  },
  universityFees: {
    fr: "Frais éventuels à l'université italienne",
    en: "Possible fees at the Italian university",
    es: "Posibles tasas en la universidad italiana",
    de: "Mögliche Gebühren an der italienischen Universität",
    it: "Possibili tasse presso l'università italiana",
  },
  universityFeesHelp: {
    fr: "Estimation des frais administratifs qui peuvent rester dus à l'université italienne selon l'établissement et le partenariat. Aucun montant n'est supposé par défaut et ce poste ne correspond pas à la tuition américaine.",
    en: "Estimate for administrative fees that may remain payable to the Italian university, depending on the institution and partnership. No amount is assumed by default, and this is not U.S. tuition.",
    es: "Estimación de las tasas administrativas que podrían seguir debiéndose a la universidad italiana según la institución y el convenio. No se presupone ningún importe por defecto y no corresponde a la matrícula estadounidense.",
    de: "Schätzung der Verwaltungsgebühren, die je nach Einrichtung und Partnerschaft noch an die italienische Universität zu zahlen sein können. Standardmäßig wird kein Betrag angenommen; dies sind keine US-Studiengebühren.",
    it: "Stima delle spese amministrative che potrebbero rimanere dovute all'università italiana, a seconda dell'istituzione e del partenariato. Nessun importo viene assunto per impostazione predefinita e non si tratta di tasse scolastiche statunitensi.",
  },
  universityFeesUsd: 0,
...
const pageCopy = {
  fr: {
    ...
    title: "Trouvez un LL.M américain via une université italienne.",
    intro:
    ...
    italianUniversity: "Université italienne",
    sheetHint: "Affine les résultats. Réinitialise à tout moment.",
  },
  en: {
    chipA: "US bar pathway",
    chipB: "Partner LL.M",
    title: "Find a U.S. LL.M. through an Italian university.",
    intro:
    ...
    italianUniversity: "Italian university",
  ...
  es: {
    ...
    title:
      "Encuentra un LL.M estadounidense a través de una universidad italiana.",
    intro:
    ...
    italianUniversity: "Universidad italiana",
    sheetHint: "Afina los resultados. Reinicia cuando quieras.",
  },
  de: {
    chipA: "Weg zur US-Anwaltszulassung",
    chipB: "Partner-LL.M.",
    title: "Finden Sie einen US-amerikanischen LL.M. über eine italienische Universität.",
    intro:
    ...
    italianUniversity: "Italienische Universität",
    sheetHint: "Ergebnisse verfeinern. Jederzeit zurücksetzen.",
  },
  it: {
    chipA: "Obiettivo abilitazione USA",
    chipB: "LL.M. partner",
    title: "Trova un LL.M. statunitense tramite un'università italiana.",
    intro:
    ...
    italianUniversity: "Università italiana",
    sheetHint: "Perfezionare i risultati. Reimposta in qualsiasi momento.",
  },
} as const

export function ItalyHomePage() {
  const all = React.useMemo(() => getAllItalianPartnerships(), [])
  const options = React.useMemo(() => getItalianFilterOptions(), [])
  const points = React.useMemo(() => getItalianUniversitiesPoints(), [])

  const [searchQuery, setSearchQuery] = React.useState("")
  const [filters, setFilters] = React.useState<FiltersState>(() =>
    emptyFilters(),
  )
  const [mapMode, setMapMode] = React.useState<"it" | "us">("it")
      ...
      <section className="pb-14">
                  ...
                  <SheetContent className="w-[92vw] overflow-y-auto sm:max-w-md">
                    ...
                    <div className="mt-6">
                      <FiltersPanel
                        ...
                        originUniversityLabel={t.italianUniversity}
                      />
                    </div>
                  </SheetContent>
                </Sheet>
          ...
          <div className="mt-6 grid gap-6 lg:mt-8 lg:grid-cols-[400px_minmax(0,1fr)]">
            <div className="order-2 space-y-5 motion-stagger lg:order-1">
              <div className="glass-panel inline-flex items-center gap-1 rounded-xl p-1">
                <Button
                  variant={mapMode === "it" ? "default" : "ghost"}
                  size="sm"
                  onClick={() => setMapMode("it")}
                  className="h-10 rounded-lg sm:h-9"
                  aria-pressed={mapMode === "it"}
                >
                  {t.italy}
                </Button>
                <Button
              ...
              {mapMode === "it" ? (
                <ItalyMap
                  points={points}
                  selectedItalianUniversity={filters.frenchUniversity}
                  onSelect={(university) =>
                    setFilters((previous) => ({
                      ...previous,
        ...
        <div id="cost-estimator" className="scroll-mt-24 pt-10 sm:pt-14">
          <CostSimulator
            partnerships={all}
            language={language}
            originConfig={italyCostSimulatorOrigin}
          />
        </div>
      </section>
    </main>
