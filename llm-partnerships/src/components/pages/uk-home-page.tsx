...
const ukCostSimulatorOrigin = {
  flights: {
    fr: "Billets d'avion Royaume-Uni - États-Unis",
    en: "United Kingdom - U.S. flights",
    es: "Vuelos Reino Unido - Estados Unidos",
    de: "Flüge vom Vereinigten Königreich in die USA",
    it: "Voli Regno Unito-USA",
  },
  universityFees: {
    fr: "Frais éventuels à l'université britannique",
    en: "Possible fees at the UK university",
    es: "Posibles tasas en la universidad británica",
    de: "Mögliche Gebühren an der britischen Universität",
    it: "Possibili tasse presso l'università britannica",
  },
  universityFeesHelp: {
    fr: "Estimation des frais qui peuvent rester dus à l'université britannique selon l'établissement et le partenariat. Aucun montant n'est supposé par défaut et ce poste ne correspond pas à la tuition américaine.",
    en: "Estimate for fees that may remain payable to the UK university, depending on the institution and partnership. No amount is assumed by default, and this is not U.S. tuition.",
    es: "Estimación de las tasas que podrían seguir debiéndose a la universidad británica según la institución y el convenio. No se presupone ningún importe y no corresponde a la matrícula estadounidense.",
    de: "Schätzung der Gebühren, die je nach Einrichtung und Partnerschaft noch an die britische Universität zu zahlen sein können. Standardmäßig wird kein Betrag angenommen; dies sind keine US-Studiengebühren.",
    it: "Stima delle spese che potrebbero rimanere dovute all'università britannica, a seconda dell'istituzione e della partnership. Nessun importo viene assunto e non si tratta della retta statunitense.",
  },
  universityFeesUsd: 0,
...
const pageCopy = {
  fr: {
    ...
    title: "Trouvez un LL.M américain via une université britannique.",
    intro:
    ...
    ukUniversity: "Université britannique",
    sheetHint: "Affine les résultats. Réinitialise à tout moment.",
  },
  en: {
    chipA: "US bar pathway",
    chipB: "Partner LL.M",
    title: "Find a U.S. LL.M. through a UK university.",
    intro:
  ...
  es: {
    ...
    title:
      "Encuentra un LL.M estadounidense a través de una universidad británica.",
    intro:
    ...
    ukUniversity: "Universidad británica",
    sheetHint: "Afina los resultados. Reinicia cuando quieras.",
  },
  de: {
    chipA: "Weg zur US-Anwaltszulassung",
    chipB: "Partner-LL.M.",
    title: "Finden Sie einen US-amerikanischen LL.M. über eine britische Universität.",
    intro:
    ...
    ukUniversity: "Britische Universität",
    sheetHint: "Ergebnisse verfeinern. Jederzeit zurücksetzen.",
  },
  it: {
    chipA: "Obiettivo abilitazione USA",
    chipB: "LL.M. partner",
    title: "Trova un LL.M. statunitense tramite un'università britannica.",
    intro:
    ...
    ukUniversity: "Università britannica",
    sheetHint: "Perfezionare i risultati. Reimposta in qualsiasi momento.",
  },
} as const

export function UkHomePage() {
  const all = React.useMemo(() => getAllUkPartnerships(), [])
  const options = React.useMemo(() => getUkFilterOptions(), [])
  const points = React.useMemo(() => getUkUniversitiesPoints(), [])

  const [searchQuery, setSearchQuery] = React.useState("")
  const [filters, setFilters] = React.useState<FiltersState>(() =>
    emptyFilters(),
  )
  const [mapMode, setMapMode] = React.useState<"uk" | "us">("uk")
      ...
      <section className="pb-14">
                  ...
                  <SheetContent className="w-[92vw] overflow-y-auto sm:max-w-md">
                    ...
                    <div className="mt-6">
                      <FiltersPanel
                        ...
                        originUniversityLabel={t.ukUniversity}
                      />
                    </div>
                  </SheetContent>
                </Sheet>
          ...
          <div className="mt-6 grid gap-6 lg:mt-8 lg:grid-cols-[400px_minmax(0,1fr)]">
            <div className="order-2 space-y-5 motion-stagger lg:order-1">
              <div className="glass-panel inline-flex items-center gap-1 rounded-xl p-1">
                <Button
                  variant={mapMode === "uk" ? "default" : "ghost"}
                  size="sm"
                  onClick={() => setMapMode("uk")}
                  className="h-10 rounded-lg sm:h-9"
                  aria-pressed={mapMode === "uk"}
                >
                  {t.uk}
                </Button>
                <Button
              ...
              {mapMode === "uk" ? (
                <UkMap
                  points={points}
                  selectedUkUniversity={filters.frenchUniversity}
                  onSelect={(university) =>
                    setFilters((previous) => ({
                      ...previous,
        ...
        <div id="cost-estimator" className="scroll-mt-24 pt-10 sm:pt-14">
          <CostSimulator
            partnerships={all}
            language={language}
            originConfig={ukCostSimulatorOrigin}
          />
        </div>
      </section>
    </main>
