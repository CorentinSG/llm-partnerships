...
import switzerlandGeojsonRaw from "../../data/switzerland.json"
...
const switzerlandGeojson = switzerlandGeojsonRaw as any
const switzerlandFeature = switzerlandGeojson.features?.[0]
...
const mapCopy = {
  fr: {
    ...
    reset: "Reset",
    aria: "Carte de la Suisse avec points des universités",
    selection: "Sélection :",
    tip: "Conseil : clique sur un point pour filtrer la liste (sur mobile : pince pour zoomer).",
  },
  en: {
    title: "Map (Switzerland)",
    clear: "Clear",
    zoomIn: "Zoom in",
    zoomOut: "Zoom out",
    resetZoom: "Reset zoom",
    reset: "Reset",
    aria: "Map of Switzerland with university points",
    selection: "Selection:",
  ...
  es: {
    ...
    reset: "Reset",
    aria: "Mapa de Suiza con puntos universitarios",
    selection: "Selección:",
...
export function SwitzerlandMap({
  points,
  selectedSwissUniversity,
  onSelect,
  className,
  language = "fr",
}: {
  points: FrenchUniversityPoint[]
  selectedSwissUniversity?: string
  ...
  const { svgPath, projectPoint } = React.useMemo(() => {
    ...
    const d = switzerlandFeature ? pathGen(switzerlandFeature) : null
    ...
    <div className={cn("w-full", className)}>
      ...
      <div className="glass-panel mt-3 rounded-2xl p-4 motion-rise">
        <div className="relative">
          ...
          >
            <defs>
              <linearGradient id="switzerlandFill" x1="0" x2="1" y1="0" y2="1">
                <stop offset="0" stopColor="hsl(var(--muted))" stopOpacity="1" />
                <stop offset="1" stopColor="hsl(var(--muted))" stopOpacity="0.55" />
              </linearGradient>
              <filter id="switzerlandSoftShadow" x="-20%" y="-20%" width="140%" height="140%">
            ...
            <g transform={`translate(${transform.x} ${transform.y}) scale(${transform.k})`}>
              {svgPath ? (
                <>
                  <path d={svgPath} fill="url(#switzerlandFill)" filter="url(#switzerlandSoftShadow)" />
              ...
              {displayPoints.map(({ p, x, y }) => {
                const selected = p.frenchUniversity === selectedSwissUniversity
      ...
      {selectedSwissUniversity ? (
        <div className="mt-3 text-sm text-muted-foreground">
          ...
          <span className="font-medium text-foreground">
            {translateDataText(selectedSwissUniversity, language)}
          </span>
        </div>
      ) : (
